...
- 任务删除函数
  ...
  - 动态任务删除举例，现有动态函数task1()、task2()
    - 在task1()中调用动态函数删除task2()
      - xTaskToDelete(task2);
      - 被删除任务中由系统分配的内存会在task1函数中释放
    - 在task1()函数中删除自身任务
      - xTaskToDelete(NULL);
      - 被删除任务task1的系统自动分配内存在空闲函数中释放
  - 静态任务删除，ts1()、ts2()
    - 在ts1任务函数中，删除任务本身，需要在删除任务之间手动释放要被删除任务的内存，然后再调用删除函数，这样可以避免内训泄漏
  - 注意：
